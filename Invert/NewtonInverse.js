...
(function() {
 function define() {
  ...
  function inverse(f, y, x, eps) {
   ...
   //assert(isFinite(y));
   //assert((fx[0][0]<=y && fx[1][0]>=y) || (fx[0][0]>=y && fx[1][0]<=y));
  ...
  ak.newtonInverse = function(f, threshold) {
   ...
   if(isNaN(threshold)) throw new Error('invalid convergence threshold in ak.newtonInverse');
