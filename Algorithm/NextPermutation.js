//AK/Algorithm/NextPermutation.js
...
(function() {
 function define() {
  ...
  ak.nextPermutation = function(a, compare, start, end) {
   var i, j, k, ai;

   if(ak.nativeType(compare)===ak.UNDEFINED_T) compare = ak.alphaCompare;
   else if(ak.nativeType(compare)!==ak.FUNCTION_T) throw new Error('invalid comparator in ak.nextPermutation');

   start = ak.arrayIndex(a, start, 'ak.nextPermutation');
   end = ak.arrayIndex(a, end, 'ak.nextPermutation');
